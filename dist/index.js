...
global.$broadcast = {
	...
	onbind: function onbind(name) {
		if (this.channels[name]) delete this.channels[name];
	},
	bind: function bind(name, ev) {
		var self = this;
		this.channels[name] = {
	...
	emitAll: function emitAll(data) {
		Object.values($broadcast.channels).map(function (channel) {
			return channel.callback(data);
		});
	}
};
...
const adapter = __webpack_require__(10)
const circularize = __webpack_require__(36)
const TesseractJob = __webpack_require__(37);
const objectAssign = __webpack_require__(35);
const version = __webpack_require__(9).version;
...
var _tesseract = __webpack_require__(15);
...
var performance = __webpack_require__(13);
var broadcast = __webpack_require__(12);

Object.prototype.service = function (serviceName) {
	var serviceCall = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
...
App.liveObj = function (data, update) {
	return new Proxy(data, {
		set: function set(target, attr, valor) {
			target[attr] = valor;
			update();
			return true;
		},
...
App.live = function (data, update) {
	data['dataSet'] = function () {
		...
		App.dataSet.apply(App, [this].concat(args));
	};
...
App.sanitaze = function (str) {
	return str.replace(new RegExp('&lt;', 'gim'), '<').replace(new RegExp('&gt;', 'gim'), '>');
};

App.render = function () {
	var tpl = _.template(App.sanitaze(App.$esEngine(App.buffer)));
	var render = tpl(App.data);
...
App.$injectorService = function () {
	var receive = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

	receive.$http = _axios2.default;
	receive.tesseract = _tesseract2.default;
	receive.broadcast = broadcast;
	receive.performance = performance;
	receive.selector = App.$;
	receive.selectorAll = App.$$;
	receive._ = _;
	receive.extends = function () {
		return Object.assign.apply(Object, arguments);
	};
	receive.arrExtend = function () {
		...
		var novo = [];
		args.map(function (a) {
			return novo = [].concat(_toConsumableArray(novo), _toConsumableArray(a));
		});
		return novo;
	};
	receive.crawler = function (url) {
...
App.initialize = function (view, data) {
	...
	_.forIn(App.data, function (value, key) {
		if ((typeof value === 'undefined' ? 'undefined' : _typeof(value)) === "object") App.data[key] = App.liveObj(value, App.update);
		if (typeof value === "function" && key !== "service") {
			var random = function random() {
				var key = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 100000;
				return Math.floor(Math.random() * key);
			};
			var gen = random();
			global['khan_' + gen + '_' + key] = function () {
				for (var _len3 = arguments.length, args = Array(_len3), _key3 = 0; _key3 < _len3; _key3++) {
					args[_key3] = arguments[_key3];
				}

				value.bind(this).apply(undefined, args.concat([App.$proxy]));
